...
object PatternMatching {
  ...
  val greeting = bob match {
    case Person(n, a) if a < 18 => s"Hi there, my name is $n and I'm $a years old"
    case Person(n, _) => s"Hello there, my name is $n and I'm not allowed to say my age"
    case _ => "I don't know who I am"
  }

  /*
  Patterns are matched in order: put most specific patterns first.
  ...
  */
  ...
  def show(expr: Expr): String = {
    expr match {
      ...
      case Prod(left @ Sum(_, _), right) => "(" + show(left) + ") * " + show(right)
