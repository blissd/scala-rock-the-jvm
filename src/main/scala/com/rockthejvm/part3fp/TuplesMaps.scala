...
object TuplesMaps {
  ...
  // tuples = finite ordered "lists" // group of values under same "big" value
  ...
  val marysPhoneNumber = phonebook.apply("Mary") // crash with NoSuchElementException if key not present
  ...
  // remove a key
  val phoneBookWithookDaniel = phonebook - "Daniel" // new map
  ...
  val aProcessedPhonebook = phonebook.map(pair => pair.copy(_1 = pair._1.toUpperCase()))
